...
void SceneParser::parsePerspectiveCamera() {
    ...
    int height = readInt();
    getToken(token);
    assert (!strcmp(token, "start"));
    float start = readFloat();
    getToken(token);
    assert (!strcmp(token, "end"));
    float end = readFloat();
    getToken(token);
    assert (!strcmp(token, "len_radius"));
    float len_radius = readFloat();
    getToken(token);
    assert (!strcmp(token, "focus"));
    float focus = readFloat();
    getToken(token);
    assert (!strcmp(token, "}"));
    camera = new PerspectiveCamera(center, direction, up, width, height, angle_radians, len_radius, focus ,start, end);
}
...
Material *SceneParser::parseMaterial() {
    ...
    material_type _type = DIFF;
    float shininess = 0, refractive = 0;
    getToken(token);
    assert (!strcmp(token, "{"));
    while (true) {
        getToken(token);
    ...
    auto *answer = new Material(diffuseColor, specularColor, shininess, attenuation, refractive, _type, filename, hasTexture);
    return answer;
}
...
Object3D *SceneParser::parseObject(char token[MAX_PARSER_TOKEN_LENGTH]) {
    ...
    } else if (!strcmp(token, "MovingObj")) {
        answer = (Object3D *)parseMovingObj();
...
MovingObj* SceneParser::parseMovingObj() {
    char token[MAX_PARSER_TOKEN_LENGTH];
    getToken(token);
    assert (!strcmp(token, "{"));
    getToken(token);
    ...
    assert (!strcmp(token, "}"));
    assert (current_material != nullptr);
    return new MovingObj(center0, center1, time0, time1, radius, current_material);
}
...
rectangle* SceneParser::parseRectangle() {
    ...
    assert (!strcmp(token, "{"));
    getToken(token);
    assert (!strcmp(token, "x0"));
    float x0 = readFloat();
    getToken(token);
    assert (!strcmp(token, "y0"));
    float y0 = readFloat();
    getToken(token);
    assert (!strcmp(token, "x1"));
    float x1 = readFloat();
    getToken(token);
    assert (!strcmp(token, "y1"));
    float y1 = readFloat();
    getToken(token);
    assert (!strcmp(token, "n"));
    float n = readFloat();
    getToken(token);
    ...
    assert (!strcmp(token, "}"));
    assert (current_material != nullptr);
    return new rectangle(x0, y0, x1, y1, n, d, current_material);
}
